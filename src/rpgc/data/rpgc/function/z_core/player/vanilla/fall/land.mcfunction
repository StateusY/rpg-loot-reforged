...
execute if block ~ ~-.1 ~ slime_block if predicate {"condition":"minecraft:entity_properties","entity":"this","predicate":{"flags":{"is_sneaking":false}}} run return run function rpgc:z_core/player/vanilla/fall/reset

# Scoreboard tracks by cm, 1block = 1meter = 100cm
scoreboard players operation @s rpgc.fall -= #25 constant
