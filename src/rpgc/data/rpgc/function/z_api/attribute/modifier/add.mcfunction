...
$function rpgc:z_api/info {message:"Added modifier $(name) with id $(id) and value $(value) from $(source)"}

function rpgc:z_core/attribute/prepare_modifier

$data remove storage rpgc:temp modifiers[{name:"$(name)",id:"$(id)",source:"$(source)"}]
$data modify storage rpgc:temp modifiers append value {name:"$(name)",id:"$(id)",type:"$(type)",value:$(value),source:"$(source)"}
