...
data modify storage bs:data load.modules append value {module:"bs.raycast",version:"3.1.1",enabled:0b}
...
execute if score $bs.raycast.major load.status matches 3 if score $bs.raycast.minor load.status matches 1 unless score $bs.raycast.patch load.status matches ..1 run return 0
